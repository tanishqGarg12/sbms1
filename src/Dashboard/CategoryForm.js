...
const CategoryForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingCategory) {
        await axios.put(`http://localhost:4000/api/v1/categoryy/categories/${editingCategory._id}`, { name, description });
  ...
  const handleDelete = async (categoryId) => {
    try {
      await axios.delete(`http://localhost:4000/api/v1/categoryy/categories/${categoryId}`);
