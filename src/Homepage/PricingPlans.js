...
const PricingPlans = () => {
  return (
    <div className="flex flex-col items-center bg-green-50 py-16 px-4 md:px-8">
      ...
      <p className="text-lg text-gray-600 mt-4 text-center max-w-2xl">
        Track your entire project from start to finish with beautiful views that make project planning a breeze. Manage your resources on a List, Box, or Gantt.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12 max-w-6xl w-full">
        {plans.map((plan, index) => (
          <div
            key={index}
            className={`rounded-lg shadow-lg p-8 ${plan.color} ${plan.textColor} flex flex-col items-center`}
          >
            ...
            <button
              className={`mt-8 py-3 px-8 rounded-full font-semibold ${plan.buttonColor} text-lg hover:opacity-90`}
            >
              Get Started
            </button>
          </div>
